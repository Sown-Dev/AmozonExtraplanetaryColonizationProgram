    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                {
                    // Calculate how many pixels should move based on time
                    float timePerFrame = 1.0 / 30.0; // 30 frames per second
                    float elapsedFrames = _Time.x / timePerFrame; // How many "30 FPS frames" have passed
                    float moveAmount = floor(elapsedFrames) * _Speed; // Only move when a full frame has passed

                    // Move by exactly 1 pixel per time interval in the specified direction
                    float2 moveOffset = _Direction.xy * moveAmount; // Move exactly one pixel at a time
